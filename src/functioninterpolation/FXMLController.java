...
public class FXMLController implements Initializable {
    ...
    protected void handleMouseClick (double posX, double posY) {
        ...
        ObservableList<XYChart.Data> datas = calculate(posX, posY);
        addFunctionToChart(datas);
    }
    
    private ObservableList<XYChart.Data> calculate(double posX, double posY) {
    ...
    private double derivativeX(double x, double y) {
        return beta * (alpha * x + y) * (delta * y + fi);
    }
    
    private double derivativeY(double x, double y){
        return gamma*Math.pow(x, 2) + fi*Math.pow(y, 2);  
    }
    ...
    private boolean isTrueParametrs() {
        ...
        try {
            ...
            if(!isNumberInRange(-100, alpha, 100)) return false;
            if(!isNumberInRange(-100, beta, 100)) return false;
            if(!isNumberInRange(-100, gamma, 100)) return false;
            if(!isNumberInRange(-100, delta, 100)) return false;
            if(!isNumberInRange(-100, fi, 100)) return false;
            if(!isNumberInRange(-100, a, b)) return false;
            if(!isNumberInRange(a, b, 100)) return false;
            if(!isNumberInRange(-100, c, d)) return false;
            if(!isNumberInRange(c, d, 100)) return false;
            if(!isNumberInRange(1, n, 10000)) return false;
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       this.numberLineChart.setAxisSortingPolicy(LineChart.SortingPolicy.NONE);
